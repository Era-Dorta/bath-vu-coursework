...
Clustering the rows of $P$, such as $P_i = \begin{bmatrix} p_{i1} & p_{i2} & p_{i3} & p_{i4}  \end{bmatrix}$ for $i = \lbrace 1, 2, 3 \rbrace$,
...
which can be solved via singular value decomposition.
...
The scene (modelled using Blender) contains some books on a self with a textured plane on the background CITE AUTHOR AND BLENDER.
...
We have presented a mathematical  several 
